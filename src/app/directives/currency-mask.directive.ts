import {
  Directive, ElementRef, HostListener, Input, forwardRef, Renderer2
} from '@angular/core';
...
export class CurrencyMaskDirective implements ControlValueAccessor {
  ...
  // Optional Parameter to allow different prefix and suffix etc.
  @Input('allowNegative') allowNegative: boolean;
  @Input('prefixString') prefixString: string = '$';
  @Input('suffixString') suffixString: string = '';
  @Input('decimalPrecision') decimalPrecision: number = 0;
  @Input('isEnableChangeEvent') isEnableChangeEvent: boolean = true;
  ...
  writeValue(value: any) {
    if (value !== this.innerValue) {
      this.el.value = this.currencyMaskService.transform(
                                value, this.allowNegative, this.decimalPrecision,
                                this.prefixString, this.suffixString);
      if (value) {
